...
from app.voice.providers.sarvam_stt import SarvamSTT
...
_SPEC_RAG_SKIP = frozenset({
    # Greetings / acks / farewells
    "yes", "no", "ok", "okay", "sure", "right", "alright", "yep", "nope",
    "hi", "hello", "hey", "bye", "goodbye", "later", "thanks", "thank",
    "good", "morning", "afternoon", "evening", "night",
    "you", "there", "anyone", "testing", "test",
    # Language-switch phrases — these are conversational, not KB queries
    "speak", "speaking", "switch", "change", "language", "translate",
    "hindi", "english", "hinglish", "marathi", "tamil", "telugu",
    "kannada", "malayalam", "gujarati", "bengali", "odia",
    "can", "we", "me", "hear", "please", "talk", "talking",
})


def _needs_rag_speculation(text: str) -> bool:
    """Return True only when the utterance is likely to need a KB lookup.

    Skips very short turns and pure greetings/acks/goodbyes so we don't waste
    an OpenAI embedding API call on every 'Yes', 'Hi', or 'Bye'.
    """
    words = [w.strip(".,?!;:") for w in text.lower().split()]
    if len(words) < 4:
        return False
    content = [w for w in words if w not in _SPEC_RAG_SKIP]
...
class VoicePipeline:
    def __init__(
        ...
        *,
        ...
        self._llm = OpenAILLM()
        stt_provider_name = runtime_cfg.get("stt_provider", "deepgram")
        self._stt_factory = SarvamSTT() if stt_provider_name == "sarvam" else DeepgramSTT()
    ...
    async def start(self) -> None:
        ...
        # Deepgram uses its own code (e.g. "en-IN", "hi", "multi").
        # Sarvam STT uses BCP-47 tags matching the TTS side (e.g. "hi-IN", "en-IN").
        if self._stt_provider_name == "sarvam":
            stt_lang = lang.get("tts_language", "hi-IN")
            # For English, Sarvam needs "en-IN" not "en"
            if stt_lang == "en":
                stt_lang = "en-IN"
        else:
            # Use Deepgram's multi-language mode when the agent supports multiple
            # languages — nova-2/nova-3 handles code-switching audio automatically.
            if len(self.cfg.languages) > 1:
                stt_lang = "multi"
            else:
                stt_lang = lang.get("deepgram_code", "en-IN")
        self._stt = self._stt_factory.stream(
            language=stt_lang, sample_rate=self.input_sample_rate
        )
        log.info("stt.provider", provider=self._stt_provider_name, language=stt_lang)
        ...
        async def _warm_rag():
            if not agent_id:
                return False
            try:
                from app.voice.providers.rag import count_chunks, warm_agent, warm_embeddings
                # Warm collection cache AND embedding TCP connection in parallel.
                await asyncio.gather(warm_agent(agent_id), warm_embeddings())
    ...
    async def _generate(self, user_text: str) -> None:
        ...
        # Gate: skip for short/conversational utterances that obviously won't
        # need a KB lookup (greetings, acks, goodbyes) to avoid wasting API calls.
        spec_rag_task: "asyncio.Task[str] | None" = None
        if self._has_knowledge and _needs_rag_speculation(user_text):
            agent_id = str(self.session_ctx.get("agent_id", ""))
            if agent_id:
