...
def _language_layer(cfg: AgentPromptConfig, active_language: str) -> str | None:
    ...
    if others:
        ...
        lines.append(
            f"You also support: {names}. "
            f"When the caller switches to another language, IMMEDIATELY reply in that "
            f"same language from your very next sentence — do not ask permission, do not "
            f"acknowledge the switch, just switch."
        )

    # Always use Western Arabic digits so TTS reads them correctly.
    lines.append(
        "Always write numbers, phone numbers, OTPs, prices, and dates using Western "
        "Arabic digits (0-9). Never use Devanagari (०-९) or other script numerals."
    )

    if lang.get("indian"):
        # When speaking an Indian language, write Indian proper nouns in the native
        # script so the TTS pronounces them with correct phonology.
        lines.append(
            "When you mention Indian names of people, places, or brands, write them in "
            "the native Devanagari (or relevant) script when speaking in an Indian "
            "language — for example write 'जूहू', not 'Juhu', when speaking Hindi."
        )
